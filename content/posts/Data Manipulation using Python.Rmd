...
```{python}
reshape2 = (product[['SIZE_(US)',"GENDER"]]
            .value_counts(dropna = True, sort = False)
            .reset_index(name = 'COUNTS')
            .pivot(index = 'SIZE_(US)', columns = 'GENDER', values = "COUNTS")
           )
```
...
```{python}
 bar_cha = (product[['SIZE_(US)',"GENDER"]]
  .value_counts(dropna = True, sort = False)
  .reset_index(name = 'COUNTS')
  .pivot(index = 'SIZE_(US)', columns = 'GENDER', values = "COUNTS")
  .fillna(0)
...
```{python}
bar_cha.reset_index(inplace = True)
```
...
```{python}
bar_cha['TOTAL_SALES'] = bar_cha['Female']+bar_cha['Male']
```
...
```{python}
# Wide to Long format using melt function from pandas 
bar_cha2 = (pd.melt(bar_cha, 
                   id_vars = 'SIZE_(US)',
                  value_vars = ['Female', 'Male'],
                  var_name = 'GENDER',
                  value_name = 'GENDERSALES')
          )
```
...
```{python}
bar_cha2['SIZE_(US)'] = bar_cha2['SIZE_(US)'].astype(str)
```
...
plt.bar(bar_cha2['SIZE_(US)'], bar_cha2['GENDERSALES'])
plt.grid(False)
```

```{python}
reshape3 = (product[['SIZE_(US)',"GENDER"]]
            .value_counts(dropna = True, sort = False)
            .reset_index(name = 'COUNTS')
            .pivot(index = 'SIZE_(US)', columns = 'GENDER', values = "COUNTS")
            .fillna(0)
...
```{python}
# Installing Plotnine in Jupyter notebook 
...
```{python}
bar_cha2.info()
bar_cha2['SIZE_(US)'] = pd.to_numeric(bar_cha2['SIZE_(US)'])
bar_cha2.info()
```
...
```{python}
(
ggplot(bar_cha2, aes(x = 'SIZE_(US)', y = 'GENDERSALES', fill = 'GENDER' ))
    + geom_bar(stat = 'identity', position = 'stack')
    + labs(x = "SHOE SIZE",
y = "TOTAL SALES",
title = "SALES OF DIFFERENT SIZES OF SHOE BY GENDER"
)+
geom_text(
...
```{python}
# Using plotnine
(ggplot(product, aes(x = 'SIZE_(US)', y= 'UNITPRICE', color = 'GENDER'))+
 facet_wrap('COUNTRY')+
